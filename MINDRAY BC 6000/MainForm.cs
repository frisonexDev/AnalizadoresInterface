...
namespace Galileo.Online
{
    public partial class MainForm : Form
    {
        ...
        private async void ConnectClient()
        {
                ...
                //await client.ConnectAsync("10.0.0.2", 5100);
                await client.ConnectAsync("172.16.16.176", 5001);
                EventWriter(true, "Conectado al analizador por  el puerto: " + "5001" + " por TCP/IP", LogEvent.Conectado, "Conectado");

                // Obtener la secuencia de entrada y salida del cliente
                ...
                stream = client.GetStream();
                    ...
                    while (true)
                    {
                        ...
                        if (bytesRead > 0)
                        {
                            ...
                            //EventWriterRxTx("MINDRAY", ASTM.WriteSpecialChars(message));
                            //EventWriterRxTxDebug("GALILEO_DEBUG", message);

                            HL7Helper helper = new HL7Helper();
                            helper.Content = message;
                            ...
                            foreach (var item in obj.messages)
                            {
                                if (item.Header.MessageType == "ORU^R01") //Envio de resultados
                                {
                                    ...
                                    ProcessOrder(item);
                                    Thread.Sleep(800);
                                    stream.Write(ASCIIEncoding.ASCII.GetBytes(ASTM.Character.ACK));
                                    EventWriterRxTx("GALILEO", ASTM.WriteSpecialChars(ASTM.Character.ACK));
                                }
                            }
        ...
        private async void ReadMessages()
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        string message = Encoding.ASCII.GetString(buffer, 0, bytesRead);
                        AddMessage("Recibido: " + message);
                    }
                }
            }
            catch (Exception ex)
            {
                AddMessage("Error al recibir mensaje: " + ex.Message);
            }
        }
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            if (conectado)
            {
                ...
                else if (ConnectionMode == "TCPCLIENT")
                {
                    Thread threadInput = new Thread(ConnectClient);
                    threadInput.Start();
                }
        ...
        private void ProcessOrder(HL7Message msg)
        {
            ...
            try
            {
                ...
                if (MuestraPorDefecto == "") // Toma del AppConfig
                {
                    BarCode = msg.ObservationRequest.FillerOrderNumber;
                ...
                {
                    BarCode = msg.ObservationRequest.FillerOrderNumber;
                    OrderNumber = DateAndTime.Now.ToString("yyyyMMdd")+ BarCode.PadLeft(4,Convert.ToChar("0")) ;
                    //OrderNumber = BarCode.PadLeft(4,Convert.ToChar("0")) ;
                ...
                foreach (var result in msg.ObservationResultList)
                {
                    ...
                    string[] dataName = result.Content.Split("^");
                    string[] dataValue = result.Content.Split("LN||");
                    var testName = result.ObservationIdentifier.Text;
            ...
            {
                EventWriter(true, "Muestra u orden invalida.", LogEvent.Enviando, "Respuesta de Galileo");
            }
