            ...
            foreach (var patient in msg.PatienInformationList)
            {
                foreach (var order in patient.OrderRecordList)
                {
                    ...
                    try
                    {
                        ...
                        OrderNumber = BarCode.Split(SeparadorMuestra)[0];
                        if (BarCode.Split(SeparadorMuestra).Length > 2)
                        {
                            CodigoMuestra = BarCode.Split(SeparadorMuestra)[1];
                        }
                        else
                    ...
                    {
                        EventWriter(true, "No se encuentra el path: " + OrderField + " en el mensaje o la orden no contiene el separador de muestra.", LogEvent.Error, "Enviando a Galileo");
                        return;
                    }
                    ...
                    foreach (var result in order.ResultRecordList)
                    {
                        ...
                        List<StargelResult> resultList = new List<StargelResult>();
                        ...
                        try
                        {
                            ...
                            foreach (var item in testName.Split(msg.header.SegmentDelimeter))
                            {
                                StargelResult r = new StargelResult();
                                r.TestName = item;
                                r.TestValue = testValue.Split(msg.header.SegmentDelimeter)[aux];
                                resultList.Add(r);
                                aux++;
                            }
                        ...
                        {
                            EventWriter(true, BarCode + ":::" + r.TestName + ":::" + r.TestValue, LogEvent.Conectado, "Procesando Resultado");
                        }

                        //EventWriter(true, BarCode + ":::" + testName + ":::" + testValue, LogEvent.Conectado, "Procesando Resultado");
                        ...
                        if (ConnectionSetup == "SERVER")
                        {
                            foreach (var item in insMngr.Instrumento.DetallesInstrumento)
                            {
                                foreach (var r in resultList)
                                {
                                    ...
                                    if (item.Homologacion == r.TestName)

                                    {
                                        ...
                                        try
                                        {
                                            rq.ValorNumero = Convert.ToDouble(r.TestValue);
                                            rq.ValorTexto = null;
