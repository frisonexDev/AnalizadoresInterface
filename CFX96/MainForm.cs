...
namespace Galileo.Online
{
    public partial class MainForm : Form
    {
        ...
        private void ReadMessage(string data)
        {
            ...
            if (Protocol == "CSV")
            {
                //MessageBox.Show("No se que hacer");

                string json = ConvertCsvToJson(data);

                List<ResultadoCFX> resultados = JsonConvert.DeserializeObject<List<ResultadoCFX>>(json);

                var qcs = resultados.Where(x => x.Sample == "CP" || x.Sample == "CN").ToList();
                bool esValido = false;

                // Validamos el QC
                if (qcs.Count == 6)
                {
                    var qcCy5Pos = qcs.Where(x => x.Fluor == "Cy5" && x.Sample == "CP").FirstOrDefault();
                    var qcCy5Neg = qcs.Where(x => x.Fluor == "Cy5" && x.Sample == "CN").FirstOrDefault();

                    var qcFAMPos = qcs.Where(x => x.Fluor == "FAM" && x.Sample == "CP").FirstOrDefault();
                    var qcFAMNeg = qcs.Where(x => x.Fluor == "FAM" && x.Sample == "CN").FirstOrDefault();

                    var qcVICPos = qcs.Where(x => x.Fluor == "VIC" && x.Sample == "CP").FirstOrDefault();
                    var qcVICNeg = qcs.Where(x => x.Fluor == "VIC" && x.Sample == "CN").FirstOrDefault();

                    

                    if (qcCy5Neg.Cqvalue() != null && qcCy5Neg.Cqvalue() <= 40)
                    {
                        if (qcCy5Pos.Cqvalue() != null && qcCy5Neg.Cqvalue() <= 40)
                        {
                            if (qcFAMNeg.Cqvalue() == null && qcVICNeg.Cqvalue() == null)
                            {
                                if (qcFAMPos.Cqvalue() != null && qcFAMPos.Cqvalue() <= 34)
                                {
                                    if (qcVICPos.Cqvalue() != null && qcVICPos.Cqvalue() <= 34)
                                    {
                                        esValido = true;
                                        EventWriter(true, "Control de Calidad Positivo", LogEvent.Conectado, "Control de Calidad");
                                    }
                                    else
                                    {
                                        EventWriter(true, "El QC de VIC Positivo no cumple el requisito", LogEvent.Conectado, "Control de Calidad");
                                    }
                                }
                                else
                                {
                                    EventWriter(true, "El QC de FAM Positivo no cumple el requisito", LogEvent.Conectado, "Control de Calidad");
                                }
                            }
                            else
                            {
                                EventWriter(true, "El QC de FAM y VIC Negativo no deben tener valor", LogEvent.Conectado, "Control de Calidad");
                            }

                        }
                        else
                        {
                            EventWriter(true, "El QC de Cy5 Positivo no cumple el requisito", LogEvent.Conectado, "Control de Calidad");
                        }
                    }
                    else
                    {
                        EventWriter(true, "El QC de Cy5 Negativo no cumple el requisito", LogEvent.Conectado, "Control de Calidad");
                    }
                ...
                {
                    EventWriter(true, "No se encontraron los suficientes parámetros de QC", LogEvent.Conectado, "Control de Calidad");
                }


                if (esValido)
                {
                    var muestras = resultados.Where(f=>f.Sample != "CP" && f.Sample != "CN").Select(e => e.Sample).Distinct().ToList();
                    foreach (var item in muestras)
                    {
                        var results = resultados.Where(x => x.Sample == item);

                        var cy5Value = results.Where(x => x.Fluor == "Cy5").FirstOrDefault().Cqvalue();
                        var famValue = results.Where(x => x.Fluor == "FAM").FirstOrDefault().Cqvalue();
                        var vicValue = results.Where(x => x.Fluor == "VIC").FirstOrDefault().Cqvalue();

                        string resultado = "";

                        if (cy5Value <= 40 && !famValue.HasValue && !vicValue.HasValue)
                        {
                            resultado = "Negativo";
                        }
                        else if (cy5Value <= 40 && (!famValue.HasValue || vicValue > 40))
                        {
                            resultado = "Indeterminado";
                        }
                        else if ((cy5Value <= 40 || !cy5Value.HasValue) && (famValue <= 40 || vicValue <= 40))
                        {
                            resultado = "Positivo";
                        }
                        else
                        ...
                        EventWriter(true, "Resultado para la muestra: " + item + " es: " + resultado + " ----> Cy5: " + cy5Value.ToString() + " FAM:" + famValue.ToString() + " VIC:" + vicValue.ToString(), LogEvent.Conectado, "Control de Calidad");
            ...
            foreach (string line in lines)
            {
                if (dataStarted)
                {
                        ...
                        Dictionary<string, string> row = new Dictionary<string, string>
                    {
                        ...
                        { "Sample", values[4] },
                        { "Cq", values[5] },
                        { "Starting Quantity (SQ)", values[6] }
                    };
                        data.Add(row);
                    }
                }
                else if (line.Trim() == "Well;Fluor;Target;Content;Sample;Cq;Starting Quantity (SQ)")
                {
                    dataStarted = true;
                }
            }
