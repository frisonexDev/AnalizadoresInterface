    ...
    public partial class MainForm : Form
    {
                    ...
                    Thread tmp_thread = new Thread(new ThreadStart(() =>
                    {
                            ...
                            while ((i = stream.Read(bytes, 0, bytes.Length)) != 0)
                        {
                            ...
                            if (data.Contains(Convert.ToChar("\x1C")))
                            {
                                ...
                                EventWriterRxTx("CLIA", ASTM.WriteTxRx(mensaje), true);
        ...
        private void ProcessOrder(HL7Message msg)
        {
            ...
            foreach (var result in msg.ObservationResultList)
            {
                ...
                var testName = result.ObservationSubId;
                          ...
                          SerialDataReceivedEventArgs e)
        {
            ...
            //EventWriter(true, "CLIA 1000: " + ASTM.WriteSpecialChars(data), LogEvent.Enviando, "Recibiendo", true);
            ...
            if (data.Contains(ASTM.Character.ENQ))
            {
                ...
                {
                    //EventWriter(true, "CLIA: " + ASTM.WriteSpecialChars(data), LogEvent.Recibiendo, "Enviando", true);

                    EventWriterRxTx("CLIA", ASTM.WriteSpecialChars(data), true);
            ...
            if (data.Contains(ASTM.Character.ACK))
            {
                ...
                if (estatus == "Enviando")
                {
                    ...
                    EventWriterRxTx("CLIA", ASTM.WriteSpecialChars(envio), true);
                    //EventWriter(true, "GALILEO: " + ASTM.WriteSpecialChars(envio), LogEvent.Enviando, "Enviando", true);
                    sp.Write(envio);
                    //Thread.Sleep(2000);
            ...
            if (mensaje.Contains(ASTM.Character.STX))
            {
                if (mensaje.Contains(ASTM.Character.ETX))
                {
                    ...
                    //EventWriter(true, "CLIA:\n" + ASTM.WriteSpecialChars(mensaje), LogEvent.Recibiendo, "Enviando", true);
                    EventWriterRxTx("CLIA", ASTM.WriteSpecialChars(mensaje), true);
