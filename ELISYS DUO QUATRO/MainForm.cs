...
namespace Galileo.Online
{
    public partial class MainForm : Form
    {
                    ...
                    if (msg.header != null && msg.header.RecordTypeId == "H")
                    {
                        ...
                        if (msg.PatienInformationList.Count > 0)
                        {
                            ...
                            ProcessOrder(msg);
                              ...
                              var queryResult = ProcessQuery(msg.Query.PatientId);

                            ASTMMessage resp = new ASTMMessage();
                            resp.header = new Utils.ASTMModel.MessageHeader(msg.header.Content);
                            resp.header.SenderNameId = "GALILEO";
                            resp.header.VersionNo = "1";
                            resp.header.TimeOfMessage = DateTime.Now;
                            resp.PatienInformationList = new List<PatientInformation>();

                            foreach (var x in queryResult)
                            {
                                PatientInformation p = new PatientInformation("P|1|PatID01|||Meier^Anna||19741001|F|||||MARTINEZ", resp.header);
                                p.PracticeAssignedPatientId = x.Identificador;
                                p.PatientName = x.ApellidoPaciente + "^" + x.NombrePaciente;
                                p.BirthDate = x.FechaNacimiento;
                                p.PatientSex = x.Genero;
                                p.AttendingPhysicianId = "";
                                p.OrderRecordList = new List<OrderRecord>();

                                int ord = 1;
                                foreach (var test in x.Resultados)
                                {
                                    OrderRecord o = new OrderRecord("O|1|SampleID02||^^^AFP||19980506|||||||||R||||||||||X",resp.header);
                                    o.SecuenceNumber = ord.ToString();
                                    o.SpecimenId = msg.Query.PatientId;
                                    o.UniversalTestIdPartOne = "^^^" + test.CodigoExamenHomologado;
                                ...
                                resp.PatienInformationList.Add(p);



                            }
                            resp.Terminator = new MessageTerminator(msg.Terminator.Content);


                            result = resp.SerializeASTM1394_97();
        ...
        private void ProcessOrder(ASTMMessage msg)
        {
            ...
            foreach (var patient in msg.PatienInformationList)
            {
                foreach (var order in patient.OrderRecordList)
                {
                    ...
                    var BarCode = patient.LaboratoryAssignedPatientId;
                    if (BarCode != null)
                    {
                          ...
                          SerialDataReceivedEventArgs e)
        {
            ...
            EventWriterRxTx("ELISYS", ASTM.WriteSpecialChars(message), true);

            if (message.Contains(ASTM.Character.ENQ))
            {
                currentState = State.Receiving;
                EventWriterRxTx("GALILEO", ASTM.WriteSpecialChars(ASTM.Character.ACK), true);
                sp.Write(ASTM.Character.ACK);
            }
            else if (message.Contains(ASTM.Character.EOT))
            {
                string result = "";
                ReadMessage(messageBuffer, ref result);
            ...
            else if (message.Contains(ASTM.Character.ACK))
            {
                if (currentState == State.Sending)
                {

                    
                    int mesgLen = resultBuffer.Split(char.ConvertFromUtf32(13),StringSplitOptions.RemoveEmptyEntries).Length;
                    string[] lines = resultBuffer.Split(char.ConvertFromUtf32(13), StringSplitOptions.RemoveEmptyEntries);
                    if (!(sendingLine == mesgLen))
                    {

                        int checksum = 0;

                        foreach (char c in lines[sendingLine] + char.ConvertFromUtf32(13) + char.ConvertFromUtf32(3))
                        {
                            checksum += c;
                        }
                        ...
                        // Convert checksum to two-character string
                        string hexChecksum = checksum.ToString("X2");


                        string msg = char.ConvertFromUtf32(2) + lines[sendingLine] + char.ConvertFromUtf32(13) + char.ConvertFromUtf32(3)+hexChecksum;





                        EventWriterRxTx("GALILEO", ASTM.WriteSpecialChars(msg), true);
                        sp.Write(msg);
                        sendingLine++;
                    }
                    else
                    {
                        EventWriterRxTx("GALILEO", ASTM.WriteSpecialChars(ASTM.Character.EOT), true);
            ...
            {
                if (message.Contains(ASTM.Character.STX) && message.Contains(ASTM.Character.ETX))
                {
            ...
            private void DataReceivedHandler_old2(
                         object sender,
                         SerialDataReceivedEventArgs e)
        {
            ...
            EventWriterRxTxDebug("DEBUG", ASTM.WriteSpecialChars(data), true);
            ...
            if (mensaje.Contains(ASTM.Character.STX))
            {
                if (mensaje.Contains(ASTM.Character.ETX))
                {
                    ...
                    if (mensaje.Contains(ASTM.Character.EOT))
                    {
                        
                            string result = "";
                            ReadMessage(mensaje, ref result);

                            if (result != "OK" && result != "Error")
                            {
                                EventWriterRxTx("ELISYS", ASTM.WriteSpecialChars(mensaje), true);
                                EventWriterRxTx("GALILEO", ASTM.WriteSpecialChars(ASTM.Character.ACK), true);
                                sp.Write(ASTM.Character.ACK);
                                estatus = "Esperando";
                                Thread.Sleep(300);
            ...
            else if (mensaje.Contains(ASTM.Character.ENQ))
            {
                EventWriterRxTx("ELISYS", ASTM.WriteSpecialChars(mensaje), true);
                EventWriterRxTx("GALILEO", ASTM.WriteSpecialChars(ASTM.Character.ACK), true);
                sp.Write(ASTM.Character.ACK);
            ...
            else if (mensaje.Contains(ASTM.Character.EOT))
            {
                if (mensaje != "")
                {
                    string result = "";
            ...
            else if (mensaje.Contains(ASTM.Character.ACK))
            {
                ...
                if (estatus == "Enviando")
                {
                    mensaje = ASTM.Character.STX + respuesta + ASTM.Character.ETX;
                    EventWriterRxTx("GALILEO", ASTM.WriteSpecialChars(mensaje), true);
                    sp.Write(mensaje);
                    Thread.Sleep(300);
                    EventWriterRxTx("GALILEO", ASTM.WriteSpecialChars(ASTM.Character.EOT), true);
                    sp.Write(ASTM.Character.EOT);
                    estatus = "Esperando Confirmación";
                    mensaje = "";
                    respuesta = "";
                }
                else if (estatus == "Esperando Confirmación")
                {
                    estatus = "Esperando";
                    mensaje = "";
                }
        ...
        private void DataReceivedHandler_old(
                           object sender,
                           SerialDataReceivedEventArgs e)
        {
        ...
        private void lblStatus_TextChanged(object sender, EventArgs e)
        {
            ...
            else if (lblStatus.Text == "Esperando")
            {
            ...
            else if (lblStatus.Text == "Esperando Confirmación")
            {
