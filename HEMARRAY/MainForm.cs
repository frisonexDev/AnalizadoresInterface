    ...
    public partial class MainForm : Form
    {
                ...
                if (server.Pending())
                {
                    ...
                    Thread tmp_thread = new Thread(new ThreadStart(() =>
                    {
                        ...
                        while ((i = stream.Read(bytes, 0, bytes.Length)) != 0)
                        {
                            ...
                            EventWriterRxTxDebug("HEMARRAY_DEBUG", ASTM.WriteSpecialChars(data), true);


                            if (data.Contains(ASTM.Character.EB))
                            {
                                messageBuffer = messageBuffer + data;
                                EventWriterRxTx("HEMARRAY", ASTM.WriteSpecialChars(messageBuffer), true);
                                ReadMessage(messageBuffer);
                                messageBuffer = "";
                            }
                            else
                            {
        ...
        private void ProcessOrder(HL7Message msg)
        {
            ...
            try
            {
                ...
                BarCode = msg.PatientId.PatientIdentifier;
           ...
           // var muestras = insMngr.GetOrdenMuestra(OrderNumber, CodigoMuestra);
            ...
            foreach (var result in msg.ObservationResultList)
            {
                ...
                try
                {
                    testName = result.ObservationIdentifier.Content;
                    testValue = result.ObservationValue.Value;
                }
                catch
                {
