                ...
                if (selected)
                {
                    ...
                    else if (((MainForm)this.Owner).Protocol.Contains("ASTM"))//Esto le estpy poniendo por la prueba lo que viene es ASTM
                    {
                        ...
                        foreach (var or in orderRq.Detalles)
                        {
                            ...
                            bool existe = false;
                            foreach (var aux in patient.OrderRecordList)
                            {
                                if (aux.UniversalTestIdPartOne == order.UniversalTestIdPartOne && order.InstrumentSpecimenId == aux.InstrumentSpecimenId)
                                {
                                    existe = true;
                                    break;
                                }
                            ...
                            //Especifico para H100
                            patientId = order.InstrumentSpecimenId;

                        }


                        patient.LaboratoryAssignedPatientId = patientId;
