...
namespace Galileo.Online
{
    public partial class MainForm : Form
    {
        ...
        private string  ReadMessage(string data)
        {
            ...
            if (Protocol == "HL7")
                ...
                if (msg.header != null && msg.header.RecordTypeId == "H")
                {
                    ...
                    if (msg.Query == null) //Si hay un Query no es un resultado.
                    {
                        ...
                        //ProcessOrder(msg);
                        EventWriter(true, "Deshabilitado el envio, revise el codigo ", LogEvent.Error, "Error");
                    }
                    else
                    {
        ...
        {
            string jsonResult = JsonConvert.SerializeObject(msg);
            EventWriter(false, JsonConvert.SerializeObject(msg, Newtonsoft.Json.Formatting.Indented), LogEvent.Esperando, "Procesando Consulta", false);

            ASTMMessage astmResponse = new ASTMMessage();

            astmResponse.header = msg.header;
            astmResponse.header.ReceiverId = astmResponse.header.SenderNameId;
            astmResponse.header.SenderNameId = "GALILEO";

            astmResponse.PatienInformationList = new List<PatientInformation>();

            PatientInformation pat = new PatientInformation();
            pat.SequenceNumber = "1";

            pat.Patient = new Utils.ASTMModel.PatienName();
            pat.PatientName = "VILLAFUERTE LUIS ALBERTO";
            
            //pat.PatientSex = ASTM.Sex.F;
            pat.Age = "40";
            pat.AgeUnits = "Y";
            pat.OrderRecordList = new List<OrderRecord>();

            OrderRecord ord = new OrderRecord();
            ord.SecuenceNumber = "1";
            ord.SpecimenId = msg.Query.SpecimenId;
            ord.UniversalTestIdPartThree = "CBC";
            ord.Priority =  ASTM.Priority.R;
            ord.RequestedOrderderedDateTime = DateTime.Now;
            ord.SpecimenCollectionDateTime = DateTime.Now;
            ord.ActionCode = "N";
            ord.SpecimenDescriptor = "Whole Blood";
            ord.OrderingPhysician = "RODRIGUEZ GALO";
            pat.OrderRecordList.Add(ord);



            astmResponse.PatienInformationList.Add(pat);

            astmResponse.Terminator = msg.Terminator;
            astmResponse.Terminator.RecordTypeId = "L";
            astmResponse.Terminator.TerminationCode = "Q";

            string sendMessage = astmResponse.Serialize();
                        ...
                        SerialDataReceivedEventArgs e)
        {
            ...
            {
                string result = ReadMessage(data); /// Ver por que pone ENQ al consultar
                if (result == "OK")
