...
namespace Galileo.Online
{
    public partial class MainForm : Form
    {
                ...
                foreach (var item in insMngr.Instrumento.DetallesInstrumento)
                {
                    if (item.Homologacion == testName)
                    {
                        foreach (var convertion in item.Conversiones)
                        {
                            ...
                            //h.Evaluate( convertion.DatoEntrada, testValue, convertion.Formula);
        ...
        private void ProcessOrder(ASTMMessage msg)
        {
            ...
            {
                JsonHelper orderJson = new JsonHelper(JsonConvert.SerializeObject(patient));
            ...
            string sender = this.Sender;
                   ...
                   List<GalileoPostResultRequest> results = new List<GalileoPostResultRequest>();
                    ...
                    var BarCode = message.GetValue(OrderField);
                      ...
                      BarCode = DateTime.Now.ToString("yyyyMMdd") + BarCode.ToString().PadLeft(4, Convert.ToChar("0"));
                    ...
                    foreach (var result in msg.resultRecord.details)
                    {
                        ...
                        foreach (var item in insMngr.Instrumento.DetallesInstrumento)
                        {
                            if (item.Homologacion == testName)
                            {
                                ...
                                EventWriter(true, JsonConvert.SerializeObject(rq), LogEvent.Enviando, "Enviando a Galileo");
                                results.Add(rq);
                        //DEsde aqui borrar si no vale la prueba
                                //EventWriter(false, JsonConvert.SerializeObject(JsonConvert.SerializeObject(results.OrderBy(x => x.Orden).ToList()), Formatting.Indented), LogEvent.Enviando, "Enviando a Galileo");
                                var resultMsg = insMngr.PostResult(JsonConvert.SerializeObject(results.OrderBy(x => x.Orden).ToList()));
                                EventWriter(true, resultMsg, LogEvent.Conectado, "Respuesta de Galileo");
                        results.Clear();
        ...
        public void AppendMessageTextBox(string value)
        {
            ...
            //msgText.Text += "\n" + text.Replace(ASTM.Character.EOT,"<EOT>").Replace(ASTM.Character.ETX,"<ETX>").Replace(ASTM.Character.STX,"<STX>").Replace(ASTM.Character.SOH,"<SOH>");
            ...
            {
                if (c.ToString() == ASTM.Character.EOT)
                {
                    //ReadMessage(sb.ToString());
                    //MessageBox.Show(sb.ToString());
                    var tt = sb.ToString()+c.ToString();
                    EventWriter(false, tt.Replace(ASTM.Character.EOT, "<EOT>").Replace(ASTM.Character.ETX, "<ETX>").Replace(ASTM.Character.STX, "<STX>").Replace(ASTM.Character.SOH, "<SOH>"), LogEvent.Recibiendo, "Recibiendo", true);
                    sb.Clear();
                    ClearMessageTextBox("");


                }
                //else if (c.ToString() == ASTM.Character.STX)
                //{
                //    sb.Clear();
                //    ClearMessageTextBox("");
            ...
            //if (text.Contains(ASTM.Character.EOT))
            //{
